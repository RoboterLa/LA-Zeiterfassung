...
def serve_static_files(filename):
    ...
    if os.path.exists(file_path):
        ...
        return jsonify({'error': f'File not found: {file_path}', 'exists': os.path.exists('build/static'), 'files': os.listdir('build/static') if os.path.exists('build/static') else []}), 404
...
def serve_react_routes(path):
    ...
    if path.startswith('static/'):
        ...
        import os
        file_path = os.path.join('build', 'static', static_path)
        if os.path.exists(file_path):
            return send_from_directory('build/static', static_path)
        else:
            return jsonify({'error': f'File not found: {file_path}', 'exists': os.path.exists('build/static'), 'files': os.listdir('build/static') if os.path.exists('build/static') else []}), 404
